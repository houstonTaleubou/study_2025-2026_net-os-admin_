...
Откроем файл `/etc/systemd/system/dhcpd.service` на редактирование и заменим в нём строку, указав интерфейс `eth1`. Затем перезагрузим конфигурацию systemd и разрешим загрузку DHCP-сервера при запуске виртуальной машины server (рис. @fig-5):
