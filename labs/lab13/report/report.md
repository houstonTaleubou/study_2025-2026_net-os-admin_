...
3. В файле `/etc/exports` настроен экспорт каталога `/srv/nfs` с доступом только на чтение для всех хостов.
   ...
   ![Настройка экспорта в /etc/exports](1.png){ #fig:001 width=80% }
...
4. Для экспортируемого каталога задан корректный контекст безопасности SELinux типа `nfs_t`, после чего изменения применены ко всей файловой системе каталога.  
   ...
   ![Настройка SELinux-контекста и применение restorecon](2.png){ #fig:002 width=80% }
...
6. Выполнена первоначальная настройка межсетевого экрана для разрешения работы службы NFS.  
   ...
   ![Разрешение службы nfs в firewalld](2.png){ #fig:003 width=80% }
...
8. В конфигурацию межсетевого экрана дополнительно добавлены службы `rpc-bind` и `mountd`, необходимые для корректной работы RPC-запросов и команды `showmount`.  
   ...
   ![Добавление служб mountd и rpc-bind в firewalld](3.png){ #fig:004 width=80% }
...
2. Выполнена попытка получения списка экспортируемых ресурсов сервера с помощью команды `showmount`.  
   ...
   ![Проверка экспортов showmount](4.png){ #fig:005 width=80% }
...
4. Проверено состояние монтирования.  
   ...
   ![Монтирование NFS-ресурса и проверка mount](5.png){ #fig:006 width=80% }

5. Для обеспечения автоматического монтирования удалённого ресурса при загрузке системы в конец файла `/etc/fstab` добавлена запись с указанием удалённого ресурса, точки монтирования и типа файловой системы NFS.  
   Использование параметра `_netdev` гарантирует, что монтирование будет выполнено только после инициализации сетевых интерфейсов.  
   Значения последних двух полей отключают резервное копирование и проверку файловой системы при загрузке.

   ![Добавление записи NFS в /etc/fstab](6.png){ #fig:007 width=80% }

6. После редактирования файла `fstab` выполнена проверка корректности конфигурации и статуса автоматического монтирования сетевых ресурсов.  
   Состояние цели `remote-fs.target` определено как активное, что подтверждает готовность системы автоматически подключать удалённые файловые системы при запуске операционной системы.

   ![Проверка статуса remote-fs.target](7.png){ #fig:008 width=80% }
...
3. На сервере выполнена проверка содержимого каталога `/srv/nfs`.  
   ...
   ![Проверка содержимого каталога /srv/nfs на сервере](10.png){ #fig:010 width=80% }

4. На клиенте выполнена проверка содержимого каталога `/mnt/nfs`.  
   В каталоге отображается подкаталог `www`, что подтверждает доступность подключённого каталога веб-сервера через NFS.

   ![Проверка содержимого каталога /mnt/nfs на клиенте](11.png){ #fig:011 width=80% }
...
5. На сервере в файле `/etc/exports` добавлена запись для экспорта каталога `/srv/nfs/www` с разрешением доступа на чтение и запись для узлов сети `192.168.0.0/16`.  
   ...
   ![Экспорт каталога /srv/nfs/www в /etc/exports](8.png){ #fig:012 width=80% }

6. Выполнено повторное экспортирование всех каталогов, указанных в файле `/etc/exports`.  
...
8. Для обеспечения сохранения bind-монтирования после перезагрузки системы на сервере в конец файла `/etc/fstab` добавлена запись для автоматического монтирования каталога `/var/www` в `/srv/nfs/www` с использованием типа `bind`.  
   ...
   ![Добавление bind-монтирования в /etc/fstab на сервере](9.png){ #fig:013 width=80% }
...
10. На клиенте выполнена финальная проверка содержимого каталога `/mnt/nfs`.  
    Каталог `www` корректно отображается и доступен, что подтверждает успешное подключение каталога веб-сервера к дереву NFS и его корректную работу после обновления настроек.
...
1. На сервере под пользователем **akabrikosov** в его домашнем каталоге создан каталог `common` с правами доступа `700`, что обеспечивает полный доступ только владельцу каталога.  
   ...
   ![Создание каталога common и файла пользователя на сервере](12.png){ #fig:014 width=80% }

2. На сервере создан каталог `/srv/nfs/home/akabrikosov`, предназначенный для экспорта пользовательского каталога через NFS.

3. Выполнено bind-монтирование каталога `/home/akabrikosov/common` в каталог `/srv/nfs/home/akabrikosov`.  
...
4. В файле `/etc/exports` добавлена запись для экспорта пользовательского каталога `/srv/nfs/home/akabrikosov` с доступом на чтение и запись для узлов сети `192.168.0.0/16`.

   ![Экспорт пользовательского каталога в /etc/exports](13.png){ #fig:015 width=80% }

5. В файл `/etc/fstab` на сервере добавлена запись для автоматического bind-монтирования каталога `/home/akabrikosov/common` в `/srv/nfs/home/akabrikosov`.  
   Это обеспечивает восстановление структуры NFS-дерева после перезагрузки системы.

   ![Добавление bind-монтирования пользовательского каталога в fstab](14.png){ #fig:016 width=80% }
...
6. Выполнено повторное экспортирование всех каталогов, указанных в `/etc/exports`.  
   ...
   ![Проверка активных монтирований на сервере](15.png){ #fig:017 width=80% }
...
8. Под пользователем **akabrikosov** на клиенте выполнён переход в каталог `/mnt/nfs/home/akabrikosov`.  
   ...
   ![Работа пользователя и ограничение доступа для root на клиенте](16.png){ #fig:018 width=80% }
...
9. На сервере выполнена проверка содержимого каталога `/home/akabrikosov/common`.  
   ...
   ![Появление файлов клиента в каталоге пользователя на сервере](17.png){ #fig:019 width=80% }
...
2. В каталоге `/vagrant/provision/server` создан исполняемый файл `nfs.sh`.  
   ...
   ![Скрипт автоматической настройки NFS на сервере](18.png){ #fig:020 width=80% }
...
4. В скрипте клиента реализована автоматическая установка пакетов NFS, создание каталога `/mnt/nfs`, монтирование экспортируемого дерева NFS с сервера и добавление записи в `/etc/fstab` для автоматического подключения при загрузке системы.
   ...
   ![Скрипт автоматической настройки NFS на клиенте](19.png){ #fig:021 width=80% }
...
В ходе выполнения лабораторной работы был настроен сервер и клиент NFSv4.  
Реализовано экспортирование каталогов с различными уровнями доступа, включая общий каталог, каталог веб-сервера и пользовательский каталог.  
Проверена корректность работы механизмов bind-монтирования, разграничения прав доступа и автоматического подключения сетевых ресурсов.  
Дополнительно выполнена автоматизация настройки окружения виртуальных машин server и client с использованием provision-скриптов Vagrant, что обеспечивает воспроизводимость и упрощает развертывание NFS-инфраструктуры.
...
3. Какую опцию следует использовать в `/etc/fstab`, чтобы убедиться, что общие ресурсы NFS могут быть установлены автоматически при перезагрузке?  
   Для корректного автоматического монтирования сетевых ресурсов NFS при загрузке системы следует использовать опцию `_netdev`.  
